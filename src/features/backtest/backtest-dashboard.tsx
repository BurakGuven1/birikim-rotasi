...
interface StrategyEntry {
  ...
  primary?: boolean;
  tacticalStats?: { pnl: number; trades: number; winRate: number; benchmarkDelta: number; turnover: number; payoffRatio: number; profitFactor: number };
}
...
export function BacktestDashboard() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      setResults([
        { name: "Çekirdek + kurallı swing", result: tactical, color: "#0f766e", primary: true, tacticalStats: { pnl: tacticalRaw.tacticalOnly.realizedPnlUsd, trades: tacticalRaw.tacticalOnly.tradeCount, winRate: tacticalRaw.tacticalOnly.winRate, benchmarkDelta: tacticalRaw.benchmarkDelta, turnover: tacticalRaw.tacticalOnly.turnover, payoffRatio: tacticalRaw.tacticalOnly.payoffRatio, profitFactor: tacticalRaw.tacticalOnly.profitFactor } },
  ...
  const primary = results.find((entry) => entry.primary);
  const tacticalStats = primary?.tacticalStats;
  ...
  return <div>
    ...
    <div className="notice section-gap"><p>Bu ekran önceki çekirdek/swing ve walk-forward modellerini simüle eder. Yeni yatırım merkezindeki profil ve katkıyla dengeleme politikasının doğrulanmış backtesti değildir. Buradaki sonuçlar gerçek hesabının gerçekleşen getirisi olarak okunmamalıdır.</p></div>
    {loading ? <Card><EmptyState title="Backtest hesaplanıyor" description="Geçmiş fiyatlar ücretsiz kaynaklardan alınıyor." /></Card> : error ? <Card><EmptyState title="Backtest tamamlanamadı" description={error} /></Card> : <>
      ...
      {primary && tacticalStats ? <div className="grid grid-4 section-gap">
        <Card className="metric-card"><div className="metric-label">Çekirdek + kurallı swing</div><div className="metric-value">{formatMoney(primary.result.finalValue, "USD")}</div><div className="metric-meta">{formatPercent(primary.result.totalReturn)} nominal USD getiri</div></Card>
        <Card className="metric-card"><div className="metric-label">ABD enflasyonu sonrası</div><div className={`metric-value ${(primary.result.realReturn ?? 0) >= 0 ? "positive" : "negative"}`}>{formatPercent(primary.result.realReturn ?? 0)}</div><div className="metric-meta">Reel USD sonuç · hedef değil, gerçekleşen</div></Card>
        <Card className="metric-card"><div className="metric-label">Pasif S&P 500 farkı</div><div className={`metric-value ${tacticalStats.benchmarkDelta >= 0 ? "positive" : "negative"}`}>{formatPercent(tacticalStats.benchmarkDelta)}</div><div className="metric-meta">Aynı tarihli nakit akışına karşı</div></Card>
        <Card className="metric-card"><div className="metric-label">Swing kalite özeti</div><div className="metric-value">{Number.isFinite(tacticalStats.profitFactor) ? tacticalStats.profitFactor.toFixed(2) : "∞"} PF</div><div className="metric-meta">{tacticalStats.trades} işlem · {formatPercent(tacticalStats.winRate)} kazanma · {formatPercent(tacticalStats.turnover)} devir</div></Card>
      </div> : null}
      ...
      <Card className="section-gap"><div className="card-title"><div><h2>Strateji ölçümleri</h2><p>USD bazlıdır; ABD TÜFE sonrası reel getiri ayrıca gösterilir. Vergi ve ürün maliyetleri dahil değildir.</p></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Strateji</th><th className="number">Yatırılan (USD)</th><th className="number">Son değer (USD)</th><th className="number">USD getiri</th><th className="number">Reel USD getiri</th><th className="number">Yıllık TWR</th><th className="number">Maks. düşüş</th><th className="number">Volatilite</th><th className="number">Swing P/L</th><th className="number">Pasif fark</th></tr></thead><tbody>{results.map((entry) => <tr key={entry.name}><td><strong>{entry.name}</strong></td><td className="number">{formatMoney(entry.result.totalInvested, "USD")}</td><td className="number">{formatMoney(entry.result.finalValue, "USD")}</td><td className="number">{formatPercent(entry.result.totalReturn)}</td><td className={`number ${(entry.result.realReturn ?? 0) >= 0 ? "positive" : "negative"}`}>{formatPercent(entry.result.realReturn ?? 0)}</td><td className="number">{formatPercent(entry.result.annualizedReturn)}</td><td className="number negative">{formatPercent(entry.result.maximumDrawdown)}</td><td className="number">{formatPercent(entry.result.volatility)}</td><td className="number">{entry.tacticalStats ? formatMoney(entry.tacticalStats.pnl, "USD") : "—"}</td><td className="number">{entry.tacticalStats ? formatPercent(entry.tacticalStats.benchmarkDelta) : "—"}</td></tr>)}</tbody></table></div></Card>
